...
func New(svc auth.Service, logger go_jwt_gmux.Logger) *LogService {
	return &LogService{
		Service: svc,
		logger:  logger,
	}
...
type LogService struct {
	auth.Service
	logger go_jwt_gmux.Logger
}

const name = "auth"

//Authenticate logging
func (ls *LogService) Authenticate(c echo.Context, user, password string) (resp go_jwt_gmux.AuthToken, err error) {
	defer func(begin time.Time) {
		ls.logger.Log(c, name, "Authenticate request", err, map[string]interface{}{
			"req":  user,
...
//Me logging
func (ls *LogService) Me(c echo.Context) (resp go_jwt_gmux.User, err error) {
	defer func(begin time.Time) {
		ls.logger.Log(
			c,
			name, "Me request", err,
